...
from tkinter import *
#importing from tkinker

expression = '' #declare expression to use throughout the program

def press(num): #updates the entry box
    global expression
    expression = expression +str(num)
    equation.set(expression)

def equalpress(): #evaluates user input
    try: #try and except in case of errors
        global expression
...
def clear(): #Clears the calculator back to nothing
    global expression
    expression = ''
    equation.set('')
...
if __name__ == "__main__":
    gui = Tk()
    gui.configure(background="light blue") #app background color
    gui.title("Ariel's Simple Calculator") #title at the top of the program
    gui.geometry("270x150") #size of the program
